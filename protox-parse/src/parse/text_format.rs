...
impl<'a> Parser<'a> {
    ...
    fn parse_text_format_field(&mut self) -> Result<ast::text_format::Field, ()> {
        ...
        let colon = match self.peek_skip_comments() {
        ...
        {
            self.add_error(ParseErrorKind::MissingColonForScalarTextFormatField {
    ...
    fn parse_text_format_message_value(&mut self) -> Result<(ast::text_format::Message, Span), ()> {
        let (delimiter, start) = match self.peek_skip_comments() {
